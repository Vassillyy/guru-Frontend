  ...
  content: {
    ...
    sections: [
      ...
      },
      {
        ...
        content:
          'Память делится на две основные области: стек (stack) и куча (heap). В стеке хранится большенство примитивных значений и ссылки на объекты, а в куче — сами объекты и некоторые примитивы. Размеры типов зависят от реализации движка и могут варьироваться.',
        addition:
          ...
          '• HeapNumber — дробные числа, числа вне диапазона SMI, NaN, Infinity, -Infinity, -0. Хранятся в куче как отдельные объекты (минимум 12 байт: заголовок + 8 байт под значение).\n' +
