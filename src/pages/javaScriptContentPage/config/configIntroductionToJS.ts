    ...
    sections: [
      {
        ...
        content:
          'JavaScript представляет высокий уровень абстракции (т.е. скрытия сложности низкоуровневых деталей) благодаря удобным конструкциям, таким как массивы, объекты, функции, классы и тд. Управление памятью осуществляется автоматически с помощью сборщика мусора (garbage collector), что освобождает разработчиков от необходимости ручного управления памятью.',
        addition:
      ...
      },
      {
        ...
        addition:
          '• Императивный подход — описание последовательности инструкций, явно указывающих, как выполнить задачу.\n' +
          ...
          '• Функциональный — организация кода вокруг функций как основных строительных блоков с поддержкой следующих принципов: неизменяемость данных (данные не изменяются, а создаются новые), чистые функции (результат зависит только от аргументов, без побочных эффектов), функции высшего порядка и декларативный стиль.',
        examples:
