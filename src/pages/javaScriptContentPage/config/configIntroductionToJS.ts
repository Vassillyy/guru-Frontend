  ...
  content: {
    ...
    sections: [
      ...
      },
      {
        ...
        content:
          'JavaScript код выполняется построчно, без предварительной компиляции в машинный код. Движок JavaScript (например, V8, SpiderMonkey) читает и выполняет код напрямую, преобразуя его в машинный код (JIT-компиляция).',
        addition:
          'JIT-компиляция (Just-In-Time) — это современный подход, который сочетает преимущества интерпретации и компиляции: код сначала интерпретируется, а часто выполняемые фрагменты кода компилируются в оптимизированный машинный код для ускорения выполнения.',
        examples:
      ...
      },
      {
        ...
        addition:
          'Императивный подход — пошаговые алгоритмы. ООП (Объектно-Ориентированное Программирование) — моделирование сущностей через классы и объекты. Функциональный — чистые функции. Прототипный — динамическое наследование без классов.',
        examples:
