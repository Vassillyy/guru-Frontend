...
console.log(admin.name); // "Администратор"`},{title:"Специальное значение NaN",content:"NaN — Not-a-Number",addition:`Специальное значение, которое означает вычислительную ошибку. Возникает при недопустимых математических операциях:
...
• Операторы равенства (==, !=):
- null == undefined → true, остальное → false
- объект → примитив
- число и строка → строка в число
- boolean → число (true→1, false→0)
- NaN → false всегда
...
• К BigInt: null → TypeError`,addition:`null и undefined при сравнении через == равны друг другу, но не равны никаким другим значениям.
...
• Нельзя повторно объявить в той же области видимости
• Нельзя переприсвоить значение, но можно изменять содержимое объектов (ссылка на объект остаётся той же, а содержимое меняется)
...
• В глобальной области видимости создаёт свойство глобального объекта
